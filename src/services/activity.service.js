...
export const createActivity = async (activityData, docente_id) => {
    ...
    const academicLoad =
        await activityRepository.findAcademicLoadById(
    ...
    // validar docente dueño
    if (academicLoad.docente_id !== parseInt(docente_id)) {
        throw new Error(
            'UNAUTHORIZED_ACADEMIC_LOAD'
        );
    }

    // validar periodo
    const period = await activityRepository.findPeriodById(activityData.periodo_id);
    ...
    const existingActivity =
        await activityRepository.findActivityByName(
            activityData.nombre,
            activityData.carga_academica_id,
    ...
    if (existingActivity) {
        throw new Error('ACTIVITY_ALREADY_EXISTS');
    }

    // validar porcentaje máximo
    const currentPercentage =
        await activityRepository
            .getTotalPercentageByPeriod(
                activityData.carga_academica_id,
                activityData.periodo_id
            );
    ...
    const newTotal =
        ...
        parseFloat(activityData.porcentaje);

    if (newTotal > 100) {
        throw new Error('PERCENTAGE_LIMIT_EXCEEDED');
    }
